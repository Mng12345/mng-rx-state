...
export namespace MngRxState {
  ...
  export const init = () => {
    ...
    combineLatest([...stateObservables, stateChangeFromUser$])
      .subscribe({
        next(values: any[]) {
...
export function createAtomState<T>({initState, key, useTimeTravel=false}: {
  initState?: T,
  key?: string,
...
export function useLocalObservable<T>(initState: T): [BehaviorSubject<T>, T, React.MutableRefObject<T>] {
  const stream$: BehaviorSubject<T> = useConstant(() => createAtomState<T>({
    initState
  }))
